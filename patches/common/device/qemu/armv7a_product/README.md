# armv7a_virt product patch set

This component patch set adds an OpenHarmony standard-system full QEMU product
named `armv7a_virt`.
...
`x86_64_virt`, but not for 32-bit ARM. The component creates the missing product
...
- keeps the standard-system service set and GPU RenderService enabled. Because
  ...
  payloads, the component builds the last OpenHarmony Mesa 21.3.3 revision before
...
```sh
bash patches/common/device/qemu/armv7a_product/apply.sh --source-root /path/to/openharmony
