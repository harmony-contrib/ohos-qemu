...
- `build_pc_uefi`: additionally generate the experimental x86_64 PC UEFI disk
  image when `x86_64_virt` is part of the build.
- `pc_disk_size`: raw disk size for the experimental PC UEFI image, default
  `32G`.
...
`scripts/package_standard_qemu.sh`. When `build_pc_uefi` is enabled, it also
uploads the generated `.qcow2` and manifest from `scripts/make_pc_uefi_disk.sh`.

## PC Installable Image Track

Directly installable PC images are a separate track from the current QEMU
artifact packaging. See `OHOS_PC_INSTALLABLE_IMAGE.md`.

The short version: `x86_64_virt` is a useful base, but a real PC image needs a
UEFI/GPT disk layout, bootloader configuration, partition installation flow, and
hardware enablement beyond QEMU virtio devices.
